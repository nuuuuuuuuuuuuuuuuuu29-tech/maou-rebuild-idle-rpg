...
const makeRecord = (overrides = {}) => ({
  id: overrides.id ?? `record-test-${recordSequence += 1}`,
...
const SUCCESS_SEED = "test-0";
const FAILURE_SEED = "test-47";
const RARE_DROP_SEED = "rare-2";
...
const simulateOneUnit = (templateId, seed = SUCCESS_SEED) => {
...
const makeDeterministicSimulationFixture = () => {
  const initial = createInitialState();
  const active = {
    id: "expedition-deterministic",
    dungeonId: "ash-border-village",
    unitIds: [initial.units[0].id],
    strategy: "balanced",
    startedAt: 1_000,
    endsAt: 31_000,
    durationSeconds: 30,
  ...
  const state = {
    ...
    units: initial.units.map((unit) => ({ ...unit, status: "expedition" })),
  };
  return { state, active };
};

test("seeded RNG reproduces a known vector and separates gameplay from presentation", () => {
  const gameplay = createSeededRng("known-seed", "gameplay");
  const repeated = createSeededRng("known-seed", "gameplay");
  const presentation = createSeededRng("known-seed", "presentation");
  const different = createSeededRng("different-seed", "gameplay");
  const expected = [
    0.7865055177826434,
    0.9706013663671911,
    0.8720422009937465,
  ...
  assert.deepEqual(Array.from({ length: 5 }, () => gameplay()), expected);
  assert.deepEqual(Array.from({ length: 5 }, () => repeated()), expected);
  assert.notDeepEqual(Array.from({ length: 5 }, () => presentation()), expected);
  assert.notDeepEqual(Array.from({ length: 5 }, () => different()), expected);
});

test("RNG helpers preserve range, inclusive integer, and pick boundary semantics", () => {
  assert.equal(randomRange(() => 0, 10, 20), 10);
  assert.equal(randomRange(() => 0.5, 10, 20), 15);
  assert.equal(randomInt(() => 0, 2, 4), 2);
  assert.equal(randomInt(() => 0.999999999, 2, 4), 4);
  assert.equal(pick(() => 0, ["first", "middle", "last"]), "first");
  assert.equal(pick(() => 0.999999999, ["first", "middle", "last"]), "last");
});

test("simulateExpeditionV1 is fully deterministic and does not mutate its inputs", () => {
  const { state, active } = makeDeterministicSimulationFixture();
  const stateBefore = structuredClone(state);
  const activeBefore = structuredClone(active);
  const first = simulateExpeditionV1(state, active, SUCCESS_SEED);
  const second = simulateExpeditionV1(state, active, SUCCESS_SEED);

  assert.deepEqual(second, first);
  assert.equal(second.record.status, first.record.status);
  assert.deepEqual(second.rewards, first.rewards);
  assert.deepEqual(second.partyUpdates, first.partyUpdates);
  assert.deepEqual(second.rescuedUnits, first.rescuedUnits);
  assert.deepEqual(second.rewards.mvp, first.rewards.mvp);
  assert.deepEqual(second.record.logs, first.record.logs);
  assert.deepEqual(second.record.battleLog, first.record.battleLog);
  assert.deepEqual(second.record.encounteredEnemies, first.record.encounteredEnemies);
  assert.deepEqual(state, stateBefore);
  assert.deepEqual(active, activeBefore);

  assert.equal(first.record.id, active.id);
  first.record.logs.forEach((entry, index) => assert.equal(entry.id, `${active.id}-log-${index}`));
  first.record.battleLog.forEach((entry, index) => assert.equal(entry.id, `${active.id}-combat-${index}`));
  assert.equal(first.rescuedUnits.length, 1);
  assert.equal(
    first.rescuedUnits[0].id,
    `${active.id}-rescue-1-${first.rescuedUnits[0].templateId}`,
  );
  assert.equal(first.rewards.rescuedUnits[0].unitId, first.rescuedUnits[0].id);
});

test("simulateExpeditionV1 matches after serializing and deserializing its inputs", () => {
  const { state, active } = makeDeterministicSimulationFixture();
  const expected = simulateExpeditionV1(state, active, SUCCESS_SEED);
  const restoredState = JSON.parse(JSON.stringify(state));
  const restoredActive = JSON.parse(JSON.stringify(active));

  assert.deepEqual(simulateExpeditionV1(restoredState, restoredActive, SUCCESS_SEED), expected);
});

test("simulateExpeditionV1 never reads global Math.random", () => {
  const { state, active } = makeDeterministicSimulationFixture();
  ...
  try {
    result = simulateExpeditionV1(state, active, SUCCESS_SEED);
    assert.equal(simulateExpedition(state, active).record.id, active.id);
  } finally {
...
test("expedition result includes detailed battle log entries", () => {
  const result = simulateOneUnit("cinder-goblin");
...
test("金額補正が遠征報酬に控えめに反映される", () => {
  const goldResult = simulateOneUnit("cinder-goblin");
  const controlResult = simulateOneUnit("bone-vanguard");
